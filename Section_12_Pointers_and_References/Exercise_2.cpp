...
Specification:
Write a C++ function find_max_element that takes an integer array and its size as input
and returns the maximum element in the array using pointers.
...
{
    int arr[] = {12, 45, 67, 23, 9};
    int size = sizeof(arr) / sizeof(arr[0]); //Total size in bytes / size of one element in bytes
